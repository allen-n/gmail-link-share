...
console.log("🚀 Building Link Share for Gmail™ by Cenva Labs extension...\n");
...
const svgFiles = readdirSync(join(__dirname, "logo")).filter((f) =>
  f.endsWith(".svg")
);
...
console.log("\n📋 Copying PNG files...");
const pngFiles = readdirSync(join(__dirname, "logo")).filter((f) =>
  f.endsWith(".png")
);

for (const file of pngFiles) {
  copyFileSync(join(__dirname, "logo", file), join(distDir, "logo", file));
  console.log(`  ✓ ${file}`);
}
...
const sourceSize =
  getDirectorySize(__dirname) -
  (existsSync(join(__dirname, "node_modules"))
    ? getDirectorySize(join(__dirname, "node_modules"))
    : 0) -
  (existsSync(join(__dirname, ".git"))
    ? getDirectorySize(join(__dirname, ".git"))
    : 0) -
  (existsSync(join(__dirname, "agents_context"))
    ? getDirectorySize(join(__dirname, "agents_context"))
    : 0);
